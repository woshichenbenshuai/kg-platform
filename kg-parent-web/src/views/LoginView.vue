<template>
  <main class="login-page">
    <section class="login-card">
      ...
      <h1>家长端</h1>
      <p class="sub">面向家长的移动网页入口，查看孩子动态、通知食谱并提交请假。</p>

      <form @submit.prevent="submit">
        <label>
          账号
          <input v-model.trim="form.username" autocomplete="username" placeholder="请输入家长账号" />
        </label>
        <label>
          密码
          <input v-model="form.password" autocomplete="current-password" type="password" placeholder="请输入密码" />
        </label>
        <button :disabled="loading">{{ loading ? '登录中...' : '进入家长端' }}</button>
      </form>
