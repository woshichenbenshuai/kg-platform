  ...
  <div class="app-container">
    <el-card class="search-header-area" shadow="never">
      <el-form :model="searchForm" label-position="left">
        <el-form-item label="学生姓名" label-width="70px" style="width: 280px">
          <el-input v-model="searchForm.studentName" placeholder="请输入学生姓名" clearable />
        </el-form-item>
        <el-form-item label="学号" label-width="45px" style="width: 240px">
          <el-input v-model="searchForm.studentNo" placeholder="请输入学号" clearable />
        </el-form-item>
        <el-form-item label="班级" label-width="45px" style="width: 240px">
      ...
      <div class="btn-box">
        ...
        <el-button class="add-btn" @click="handleCreate">新增学生</el-button>
      </div>
    </el-card>
    ...
    <el-drawer :title="drawerTitle" v-model="drawerVisible" size="500px" direction="rtl">
      <el-form :model="formData" label-position="right" label-width="95px" style="padding: 0 20px">
        <el-form-item label="学号："><el-input v-model="formData.studentNo" placeholder="请输入学号" /></el-form-item>
        <el-form-item label="学生姓名："><el-input v-model="formData.studentName" placeholder="请输入学生姓名" /></el-form-item>
        ...
        <el-form-item label="生日：">
          <el-date-picker v-model="formData.birthday" type="date" value-format="YYYY-MM-DD" placeholder="请选择生日" style="width: 100%" />
        </el-form-item>
...
import { fetchClasses } from '@/api/kinder/class'
import { createStudent, deleteStudent, fetchStudents, updateStudent } from '@/api/kinder/student'
...
const searchForm = reactive({ studentName: '', studentNo: '', classId: '' as number | string })
const drawerVisible = ref(false)
const drawerTitle = ref('')
const formData = ref<any>({})

const loadClasses = async () => {
  const res = await fetchClasses({ current: 1, size: 200, status: 1 })
  if (res.data.code === '0') classOptions.value = res.data.data?.records || []
}

const className = (classId: number | string) => classOptions.value.find(item => String(item.id) === String(classId))?.className || '-'

const getList = async () => {
  loading.value = true
  try {
    const res = await fetchStudents({ current: listQuery.current, size: listQuery.size, ...searchForm })
    if (res.data.code === '0') {
...
const submitForm = async () => {
  if (formData.value.id) await updateStudent(formData.value)
  else await createStudent(formData.value)
...
const handleDelete = (row: any) => {
  ElMessageBox.confirm(`确认删除学生“${row.studentName}”？`, '提示信息', { type: 'warning' }).then(async () => {
